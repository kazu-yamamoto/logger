...
getSourceIP (FromHeaderCustom hn) = fromMaybe "-" . getSourceFromHeaderCustom hn
...
getSourceFromHeaders headerNames req = addr
  where
    maddr = find (\x -> fst x `elem` headerNames) hdrs
    addr = fmap snd maddr
    hdrs = requestHeaders req

getSourceFromHeaderCustom :: HeaderName -> Request -> Maybe ByteString
getSourceFromHeaderCustom hn = getSourceFromHeaders [hn]
